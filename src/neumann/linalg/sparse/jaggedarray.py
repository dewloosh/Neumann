...
def cut(shp): return np.full(shp[0], shp[1], dtype=int)
...
class JaggedArray(NDArrayOperatorsMixin, Wrapper):
    ...
    the argument `force_numpy=False`, the data is stored as an Awkward array:
    ...
    >>> JaggedArray(data, cuts=[3, 3, 4]).to_numpy()
    array([[ 1,  2,  3,  0],
           [ 4,  5,  6,  0],
           [ 7,  8,  9, 10]]) 
    ...
    def __init__(self, data: Iterable = None, *, cuts: Iterable = None, 
                 force_numpy:bool=True, **kwargs):
    ...
    def __setitem__(self, key):
        return self._wrapped.__setitem__(key)
